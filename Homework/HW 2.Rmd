...
43.
```{r}
cat(" The smallest integer is -2^(32-1)=", as.character(-2^(32-1)), " while the largest integer is 2^(32-1)-1=", as.character(2^(32-1)-1), "\n")

```

44.
```{r}
cat("The numbers that correspond to the given k-bit sequences are -1 and -2^(k-1)+1, which are the biggest negative number and the second smallest negative number, respectively. \n")
...
binary_to=function(n){
  ...
  binary=integer(p+1)
  for(i in 0:p){
    if(n>=2^(p-i)){ # start with highest power that fits into n
      binary[i+1]=1 # fill vector from left to right
      n=n-2^(p-i)   # substract power from n and go again
    }
  }
  return(binary)
}
...
to_twos_comp=function(n, k=32){
  if(n>=0){
    "Twos complement representation for positive numbers is just the regular presentation."
    return(c(rep(0, k-length(binary_to(n))), binary_to(n)))                       
  }else{
    "For negative numbers its 1 as a \"sign\" bit and the binary representation of 2^k minus the absolute value of the number."
...
to_twos_comp(-2, 3)
to_twos_comp(-1, 3)
from_twos_comp=function(binary){
  if(binary[1]==0){
    "straightforward reversal of binary"
    return(binary_from(binary))
  }else{
    "reverse the transformation operations (leave out first bit and then substract 2^(length of remainder) from binary representation."
    return(binary_from(binary[-1])-2^(length(binary)-1))
  }
}
from_twos_comp(to_twos_comp(-5, 4))
from_twos_comp(c(1, 1, 1, 1, 1, 1, 1, 1))
from_twos_comp(c(1, 0, 1, 1, 0, 1, 0, 1))
```
46.
